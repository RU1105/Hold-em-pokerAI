		...
		success: function(data)
		{
			setTimeout(function(){
				...
				if(data == 2){	//加注
					...
					if(enemy_raise > my_chip_remind){
						enemy_raise = my_chip_remind;
					}
...
$('.chip_botton').click(function(){ //加注
	var add_raise = $(this).html(); //抓取加注的按鈕金額
	add_raise = parseInt(add_raise, 10) 
	var now_raise = $('.chip_raise').html(); //抓取當前加注的籌碼金額
	now_raise = parseInt(now_raise, 10);
	var final_raise = add_raise + now_raise;
	var my_chip_value = $(".my_chip .my_chip_value span").html();
	my_chip_value = parseInt(my_chip_value, 10);
	var enemy_max_chip = $('.table_top_input').val();
	enemy_max_chip = parseInt(enemy_max_chip, 10);
	var my_max_chip = $('.table_bottom_input').val();
	...
	if(final_raise > max_chip){
		$('.chip_raise').html(max_chip);
	}
	else{
		$('.chip_raise').html(final_raise);
	}
});
$('.chip_botton_confirm').click(function(){ //確定的按鈕功能
	var enter_raise = document.getelementbyid('.chip_raise_numby').value
